...
def use_address(request,id):
    ...
    #setting an address as default and making all other address as default=false
    ...
    if request.method =='POST':
        ...
        except use_address.DoesNotExists:
            pass
...
def order_payment(request,coupon_id,coupon_amount =0,applied_coupon =0 ,total=0,quantity=0,wallet=0):
    try:
        applied_coupon = Coupon.objects.get(id=coupon_id)
        coupon_amount = applied_coupon.discount_price
    except:
        pass
    ...
    tax=(2*total)/100
    grand_total = total +tax -float(coupon_amount)
    ...
    if request.method == 'POST':
        ...
        if payment_type =="Wallet":
            try:
                ...
                messages.info(request,"Insufficient Wallet Amount")
                return redirect('checkout')
            if grand_total <= wallet.wallet_amount:
                for cart in cart_items:
                    if cart.colour.colour_stock >0:
                        ...
                        data = Order(
                            ...
                            product_id = cart.product_name.id,
                            colour_id =cart.colour.id,
                            quantity =cart.quantity,
                            bulk_order_id = bulk_order_id,
                            unit_amount = cart.product_name.regular_price,
                        ...
                        update_quantity = cart.colour.colour_stock -new_qty
                        cart_item.colour.colour_stock = update_quantity
                        cart.colour.save()
        ...
        if payment_type == "cod":
            ...
            return redirect('order_summary', bulk_order_id=bulk_order_id)
        
        elif payment_type == "razorpay":
            print(255)
            amounts = str(grand_total)
            name = full_name
